 ...
 * 父子 Span 示例
 *
 * <p>展示如何创建父子关系的 span，包括： - 创建父子关系的 span - 展示 context 传递机制 - 包含异步任务的追踪示例
 *
...
public class ParentChildSpanExample {
   ...
   *
   * <p>注意：在实际应用中，OpenTelemetry 的 context 传播是自动的。 在同一个线程中，在父 span 的 scope 内创建的子 span 会自动成为父子关系。
   * 但在异步任务中，需要手动传递 context。
   */
  private static CompletableFuture<Void> asyncRendering(
      CozeLoopClient client, CozeLoopSpan parentSpan) {
    // 获取当前 context（包含父 span 信息）

    return CompletableFuture.runAsync(
        () -> {
          // 在新的线程中设置 context
          try (CozeLoopSpan asyncSpan =
              client.startSpan("asyncRendering", "rendering", parentSpan)) {
            // 模拟异步处理
            try {
              Thread.sleep(1000);
            } catch (InterruptedException e) {
