 ...
 *
 * <p>重要提示：Personal Access Token 不够安全，仅用于测试环境。 生产环境请使用 OAuth JWT 认证方式。
 *
 * <p>使用前请先设置以下环境变量： - COZELOOP_WORKSPACE_ID: 你的工作空间 ID - COZELOOP_API_TOKEN: 你的访问令牌
 *
 * <p>创建令牌的步骤： 1. 访问 https://www.coze.cn/open/oauth/pat 2. 创建新的令牌 3. 妥善保管你的令牌，防止数据泄露
 */
  ...
  private static void useCustomClient(String workspaceId, String apiToken) {
    ...
    try {
      ...
      try (CozeLoopSpan span = client.startSpan("custom_span", "custom")) {
        span.setTag("example", "pat_custom_config");
