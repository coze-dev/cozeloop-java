...
class AuthInterceptorTest {
  ...
  void testInterceptAddsHeaders() throws IOException, InterruptedException {
    ...
    okhttp3.OkHttpClient client =
        new okhttp3.OkHttpClient.Builder().addInterceptor(interceptor).build();

    Request request = new Request.Builder().url(mockWebServer.url("/test")).get().build();
    ...
    assertThat(recordedRequest.getHeader("X-Coze-Client-User-Agent"))
        .isEqualTo(UserAgentUtils.getClientUserAgent());
  }
}
