 ...
 *
 * <p>This class provides a bridge between CozeLoop SDK and OpenTelemetry, managing the complete
 * trace lifecycle from span creation to export. It configures and initializes the OpenTelemetry SDK
 * with OTLP/HTTP exporter and BatchSpanProcessor.
 *
 ...
 * propagation, ensuring that trace context (trace ID, span ID, baggage) is automatically propagated
 * to child spans within the same thread and across async boundaries.
 *
 * <p><b>Example Usage:</b>
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p><b>Shutdown:</b> Always call {@link #shutdown()} when the application is shutting down to
 * ensure all pending spans are flushed and exported:
 *
 * <pre>{@code
...
public class CozeLoopTracerProvider {
  ...
  private CozeLoopTracerProvider(
    ...
    this.sdkTracerProvider =
        SdkTracerProvider.builder().setResource(resource).addSpanProcessor(batchProcessor).build();
    ...
    logger.info(
        "CozeLoop TracerProvider initialized with service: {}, workspace: {}, endpoint: {}",
        serviceName,
        workspaceId,
        spanEndpoint);
  }
   ...
   *
   * <p>The instrumentation name identifies the library or framework that is creating spans. This
   * helps organize spans in the CozeLoop platform.
   *
   * <p>Example:
   *
   * <pre>{@code
   ...
   * }</pre>
   *
   * @param instrumentationName the name of the instrumentation library (e.g., "cozeloop-java-sdk",
   *     "my-application")
   ...
   *
   * <p>This provides direct access to the OpenTelemetry TracerProvider for advanced use cases that
   * require OpenTelemetry-specific APIs.
   *
   * <p>Example:
   *
   * <pre>{@code
  ...
  /**
   * Force flush all pending spans. This method waits up to 10 seconds for the flush to complete.
   */
  public void flush() {
   ...
   *
   * <ul>
   *   <li>All pending spans are flushed from the queue
   *   <li>All spans are exported to CozeLoop platform
   *   <li>Resources are properly released
   * </ul>
   *
   * <p><b>Important:</b> After shutdown, the TracerProvider cannot be used again. You must create a
   * new instance if needed.
   *
   * <p>The shutdown process:
   *
   * <ol>
   *   <li>Force flush all pending spans (waits up to 10 seconds)
   *   <li>Shutdown the TracerProvider (waits up to 10 seconds)
   *   <li>Shutdown the SpanExporter
   * </ol>
   *
   * <p>Example:
   *
   ...
   *
   * <ul>
   *   <li><b>maxQueueSize</b>: Maximum number of spans that can be queued before spans are dropped
   *       (default: 2048)
   *   <li><b>batchSize</b>: Maximum number of spans per batch sent to exporter (default: 512).
   *       Note: CozeLoopSpanExporter further splits into batches of 25
   *   <li><b>scheduleDelayMillis</b>: Time between automatic batch exports (default: 5000ms = 5
   *       seconds)
   *   <li><b>exportTimeoutMillis</b>: Maximum time to wait for export to complete (default: 30000ms
   *       = 30 seconds)
   * </ul>
   *
   ...
   *
   * <ul>
   *   <li><b>High Throughput</b>: Increase maxQueueSize and batchSize
   *   <li><b>Low Latency</b>: Decrease scheduleDelayMillis
   *   <li><b>Network Issues</b>: Increase exportTimeoutMillis
   * </ul>
   */
